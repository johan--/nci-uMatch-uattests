...
#    Then the patient status is changed to "REQUEST_NO_ASSIGNMENT" for patient "<patient_id>"
  ...
  Scenario Outline: Patient on a treatment arm receives a REQUEST ASSIGNMENT message from COG with rebiopsy flag set to Y
    ...
    And assay message is received from MDA with biomarker "<PTEN>", surgical event id "<se_id>" and result "POSITIVE" for the patient "<patient_id>"
    And assay message is received from MDA with biomarker "<MLH1>", surgical event id "<se_id>" and result "POSITIVE" for the patient "<patient_id>"
