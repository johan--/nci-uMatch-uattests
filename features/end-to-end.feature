...
Feature: End-to-End patient assignment process tests
  ...
  Background:
    Given Auth0 token is generated
  ...
  Scenario Outline: Assign a Patient to a Treatment Arm
    ...
    And assay message is received from MDA with biomarker "<BAF47>", surgical event id "<se_id>" and result "INDETERMINATE" for the patient "<patient_id>"
    And assay message is received from MDA with biomarker "<BRG1>", surgical event id "<se_id>" and result "INDETERMINATE" for the patient "<patient_id>"
    ...
    Examples:
      | patient_id | se_id    | mol_id | slide_code | PTEN     | BAF47     | BRG1     | vcfFile    | analysis_id | treatment_arm:stratum |
      | E2E-01    | event-01 | mol-01 | sb-01      | ICCPTENs | ICCBAF47s | ICCBRG1s | E2E-01.vcf | job1        | APEC1621-A:100        |
  ...
  Scenario Outline: A treatment arm is chosen by MATCHBox, but COG sends a REQUEST ASSIGNMENT message
    ...
    Examples:
      | patient_id | se_id    | mol_id | slide_code | PTEN     | BAF47     | BRG1     | vcfFile    | analysis_id | treatment_arm:stratum |
      | E2E-02     | event-02 | mol-02 | sb-02      | ICCPTENs | ICCBAF47s | ICCBRG1s | E2E-02.vcf | job2        | APEC1621-A:1          |
  ...
  Scenario Outline: Patient on a treatment arm receives a REQUEST ASSIGNMENT message from COG with rebiopsy flag set to N
    ...
    Examples:
      | patient_id | se_id    | mol_id | slide_code | PTEN     | BAF47     | BRG1     | vcfFile    | analysis_id | treatment_arm:stratum     |
      | E2E-03     | event-03 | mol-03 | sb-03      | ICCPTENs | ICCBAF47s | ICCBRG1s | E2E-03.vcf | job3        | SNV_location_intronic:100 |
  ...
  Scenario Outline: Patient on a treatment arm receives a REQUEST ASSIGNMENT message from COG with rebiopsy flag set to Y
    ...
    And assay message is received from MDA with biomarker "<BAF47>", surgical event id "<se_id>" and result "POSITIVE" for the patient "<patient_id>"
    And assay message is received from MDA with biomarker "<BRG1>", surgical event id "<se_id>" and result "POSITIVE" for the patient "<patient_id>"
    ...
    Then the user logs out of MATCHBox
    Examples:
      | patient_id | se_id    | mol_id | slide_code | PTEN     | BAF47     | BRG1     | vcfFile    | analysis_id | treatment_arm:stratum | se_id1     | mol_id1  | slide_code1 | analysis_id1 | vcfFile1     | treatment_arm:stratum1    |
      | E2E-04     | event-04 | mol-04 | sb-04      | ICCPTENs | ICCBAF47s | ICCBRG1s | E2E-04.vcf | job4        | APEC1621-A:100        | event-04-1 | mol-04-1 | sb-04-1     | job4-1       | E2E-04-1.vcf | SNV_location_intronic:100 |
  ...
  Scenario Outline: MATCHBox finds a matching treatment arm for a patient, but the treatment arm is in SUSPENDED state
    ...
    Then the user logs out of MATCHBox
    Examples:
      | patient_id | se_id    | mol_id | slide_code | PTEN     | BAF47     | BRG1     | vcfFile    | analysis_id |
      | E2E-05     | event-05 | mol-05 | sb-05      | ICCPTENs | ICCBAF47s | ICCBRG1s | E2E-05.vcf | job5        |
